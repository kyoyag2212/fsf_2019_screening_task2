...
class Example(QMainWindow):
    ...
    def _plot_chart(self,plot_data,xLabel,yLabel,anotationLabel,chart = 'line'):
        ...
        # plot data
        if chart == 'line':
            ax.plot(xLst,*yLst) # unpack the list which is form of [ [y1,y2,y3],<marker_here>,[y1,y2,y3],<marker_here ] to [y1,y2,y3],<marker_here>,[y1,y2,y3],<marker_here
        elif chart == 'scatter':
            ax.plot(xLst,*yLst)
        elif chart == 'sline':
            print(yLst)
        ...
        for an in range(len(anotationLabel)): 
            ax.annotate(anotationLabel[an],xy=(0.1,yLst[an if an%2==0 else an+1][0])) # x co-ordinate is fixed at 0.1 while y co-ordinate is the first value in the column
        self.canvas.draw()
